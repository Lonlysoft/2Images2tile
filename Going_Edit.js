...
function general(imagemSdw){
	...
	for(let i = 0; i < imagemSdw.height; i++){
		...
		for(let j = 0; j < imagemSdw.width; j++){
			...
			mapa[i].push(idOf(coresTemplate, corDoPixel[i][j]));
		}
	}
}
...
function generalForGraphicPurposes(imagemSdw){
	let corDoPixel = []
	let imageLengthInTileSize = Math.floor(imagemSdw.width/TileSize);
	ctx.drawImage(imagemSdw, 0, 0, imagemSdw.width, imagemSdw.height);
	for(let i = 0; i < imagemSdw.height; i++){
		mapa.push(new Array());
		corDoPixel.push(new Array());
		for(let j = 0; j < imagemSdw.width; j++){
			corDoPixel[i].push(obterCorDoPixel(ctx, j, i));
			let fret = 5 + idOf(coresTemplate, corDoPixel[i][j]);
			mapa[i].push(fret*4);
		}
	}
	for(let i = 0; i < mapa.length; i++){
		for(let j = 0; j < mapa[i].length; j++){
			...
			if(flag[0] && flag[1] && flag[2] && flag[3])
				...
				mapa[i][j] -= imageLengthInTileSize-1;
				...
				mapa[i][j] -= imageLengthInTileSize;
